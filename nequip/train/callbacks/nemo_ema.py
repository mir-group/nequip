# adapted from the NVIDIA NeMo Framework
# https://github.com/NVIDIA/NeMo/tree/main
# changes made in order to avoid https://github.com/Lightning-AI/pytorch-lightning/issues/17485
# and to include use_num_updates from pytorch_ema https://github.com/fadel/pytorch_ema/tree/master
...
class NeMoExponentialMovingAverage(Callback):
    """Exponential Moving Averaging (EMA) implementation from `NeMo <https://github.com/NVIDIA/NeMo/tree/main>`_.

    When training a model, this callback will maintain moving averages of the trained parameters. When evaluating, the moving averages of the trained parameters are used. When saving, an additional set of parameters with the prefix ``ema`` is saved.
    ...
    Args:
        decay (float): the exponential decay used when calculating the moving average (has to be between 0 and 1)
        validate_original_weights (bool): validate using the original weights, as apposed to the EMA weights
        every_n_steps (int): number of steps between each EMA update
        cpu_offload (bool): offload weights to CPU
        use_num_updates (bool): whether to dampen EMA effects at earlier epochs
    """
    ...
    def _should_validate_ema_weights(self, trainer: lightning.Trainer) -> bool:
        return not self.validate_original_weights and self._ema_initialized(trainer)

    def _ema_initialized(self, trainer: lightning.Trainer) -> bool:
        return any(
            isinstance(optimizer, EMAOptimizer) for optimizer in trainer.optimizers
        )

    def swap_model_weights(
        self, trainer: lightning.Trainer, saving_ema_model: bool = False
    ):
    ...
    def save_ema_model(self, trainer: lightning.Trainer):
        """
    ...
    @contextlib.contextmanager
    def save_original_optimizer_state(self, trainer: lightning.Trainer):
        for optimizer in trainer.optimizers:
            assert isinstance(optimizer, EMAOptimizer)
...
class EMAOptimizer(torch.optim.Optimizer):
    ...
    def update(self):
        ...
        # consider if num_updates is used
